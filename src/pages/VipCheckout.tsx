...
const APP_RETURN_DEEP_LINK = import.meta.env.VITE_VIP_RETURN_DEEP_LINK || "zztherapy://vip";
...
const VipCheckout = () => {
  ...
  const [message, setMessage] = useState("Preparing VIP checkout...");
  ...
  useEffect(() => {
    ...
    const startCheckout = async () => {
      ...
      try {
        ...
        const orderResp = await fetch(`${apiBaseUrl}/vip/checkout/create-order`, {
          method: "POST",
        ...
        const rzp = new window.Razorpay({
          ...
          handler: async (response: unknown) => {
            ...
            try {
              const verifyResp = await fetch(`${apiBaseUrl}/vip/checkout/verify`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  checkoutToken,
              ...
              setStatus("success");
              setMessage(verifyData.data?.message || "VIP activated!");
              setAppOpenUrl(
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 to-pink-50 p-6">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center space-y-4">
        <h1 className="text-2xl font-bold text-purple-900">VIP Membership</h1>
